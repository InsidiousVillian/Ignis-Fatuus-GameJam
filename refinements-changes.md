...
- **Dash movement phase (Input Priority fix)** — inside `Player.update()`, the standard `this.x = mouseX` / `this.y = mouseY` assignment is placed in the `else` branch, meaning it is wholly skipped while `dashActive` is `true`. The direction vector is captured once at the moment `triggerDash()` is called and stored as fixed `dashVx`/`dashVy` scalars; subsequent mouse movement has no effect until the dash completes. Each frame, `dashVx`/`dashVy` is applied to the position, `dashFrames` is decremented, and `dashActive` is set to `false` when `dashFrames` reaches 0. Viewport clamping runs regardless, preventing the dash from clipping outside the canvas edge.

- **Ghost trail rendering (post-move ghost fix)** — the ghost is recorded *after* applying the velocity for that frame, not before. This ensures each `{x, y}` entry marks the player's **actual coordinates during that dash frame** rather than the pre-move position. The initial ghost seeded in `triggerDash()` marks the departure point; the four subsequent ghosts (one per dash frame) trace the arrival path without duplicating the start position. `Player.draw()` renders each ghost as a screen-blended radial gradient halo plus a solid cyan disc. Opacity and radius both scale with `ghost.life / DASH_GHOST_LIFE`, giving the most recently deposited copy the brightest and largest appearance — reading as a directional smear pointing away from the player.

- **Input binding (Shift — dedicated listener)** — the Shift check is extracted from the combined `window.addEventListener('keydown', ...)` handler into its own standalone listener. The condition is `e.shiftKey && e.key === 'Shift'`: `e.shiftKey` provides layout-independent detection of the modifier (satisfying cross-keyboard compatibility), while `e.key === 'Shift'` restricts the event to the actual Shift keydown, preventing Shift+F or Shift+Space from firing both their primary binding and a dash simultaneously. `e.preventDefault()` suppresses the browser's own Shift-key behaviour (text selection, scroll-select) during gameplay.

- **Context Menu Prevention** — the `canvas.addEventListener('contextmenu', ...)` handler calls `e.preventDefault()` as its very first operation before the gameState check. This unconditionally suppresses the browser's native right-click context menu, which would otherwise steal focus from the canvas, suspend `mousemove` events, and effectively freeze the player cursor mid-game until the menu is dismissed.
...
**AI Collaboration Note:** The ghost trail was initially implemented using pre-move position captures (pushing position *before* applying `dashVx/Vy`). This introduced a duplicate ghost at the departure point (the start position was recorded both in `triggerDash()` and on the first update frame before any movement). The approach was corrected to post-move capture: velocity is applied first, then the ghost is recorded, so each ghost entry reflects the player's actual rendered position during that frame. The departure point ghost in `triggerDash()` is retained as the origin marker. AI recommended the high-pass filter on `_sfxDash` to prevent low-frequency content from masking the background drone, arguing that a full-spectrum chirp would "compete with the ambient bass register" — matching Farnell's (2010:ch.4) guidance on SFX spectral separation in mixed audio environments.
